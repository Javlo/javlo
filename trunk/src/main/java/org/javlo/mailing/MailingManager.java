 ...
 * It will send mails to a bunch of recipients one by one, making it personalized.
 * 
 * @author plemarchand
 */
public class MailingManager {
	...
	private static int countMail = 0;
     ...
     * 
     * @param props Properties with necessary parameters to connect to the smtp server
     * 		in /WEB-INF/config/mailing.properties
     * 
     ...
     */
    public void sendMailing(InternetAddress sender,
     ...
     * @param recipients all recipients, each one will receive the message with its address in the "To" field
     * @param optional "Bcc" recipients if only 1 recipient in recipients[],
     * address used to send notification on start / end mailing, set to null if none required
     * @param subject the Subject of the message
     * @param content the Content of the message
     * @param html flag indicating wether the Content is html (true) or text (false)
     * @throws IllegalArgumentException if no recipient provided or no sender
     */
    public String sendMailing(InternetAddress sender,
            InternetAddress[] recipients,
            InternetAddress[] bccRecipients,
            String subject,
            String content,
        ...
        logger.info("using: " + props.getProperty(SMTP_HOST_PARAM, DEFAULT_SMTP_HOST)
                + " / " + props.getProperty(SMTP_USER_PARAM) + " / " + props.getProperty(SMTP_PASSWORD_PARAM));
        ...
        try {
	        ...
	        if (recipients.length > 1) {
		        ...
		        startTransport.connect(props.getProperty(SMTP_HOST_PARAM, DEFAULT_SMTP_HOST),
            ...
            for (int i = 0; i < numRecip; i++) {
                ...
                try {
                    ...
                    transport.connect(props.getProperty(SMTP_HOST_PARAM, DEFAULT_SMTP_HOST),
                            props.getProperty(SMTP_USER_PARAM),
                            props.getProperty(SMTP_PASSWORD_PARAM));
                    ...
                    logger.info("mail " + String.valueOf(i + 1) + " of " + numRecip
                            + " sent to: " + recipients[i].getAddress());
            ...
            if (recipients.length > 1) {
		        ...
		        endTransport.connect(props.getProperty(SMTP_HOST_PARAM, DEFAULT_SMTP_HOST),
	                    props.getProperty(SMTP_USER_PARAM),
	                    props.getProperty(SMTP_PASSWORD_PARAM));
    ...
    /**
     * @param sender the "From" field
     * @param recipient address in the "To" field
     * @param bcc address in the "bcc" field (for control)
     * @param subject the Subject of the message
     * @param content the Content of the message
     * @param isHTML flag indicating wether the Content is html (true) or text (false)
     * @throws IllegalArgumentException if no recipient provided or no sender
     */
     ...
     * @param sender the "From" field
     * @param recipient address in the "To" field
     * @param bccAddress addresses in the "bcc" field (for control)
     * @param subject the Subject of the message
     * @param content the Content of the message
     * @param isHTML flag indicating wether the Content is html (true) or text (false)
     * @throws IllegalArgumentException if no recipient provided or no sender
