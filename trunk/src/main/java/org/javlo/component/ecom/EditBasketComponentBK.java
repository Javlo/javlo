 ...
 */
public class EditBasketComponentBK extends AbstractPropertiesComponent implements IAction {
	...
	public void renderBasket(ContentContext ctx, PrintStream out, boolean interactive) throws Exception {
		...
		if (products.size() == 0) {
			...
			for (Product product : products) {
				...
				if ((product.getImage() != null) && (product.getImage().getImageLinkURL(ctx) != null)) {
					ctx.setAbsoluteURL(true);
					String url = URLHelper.createTransformURL(ctx, getPage(), product.getImage().getImageLinkURL(ctx), "basket");
