...
<c:forEach var="child" items="${menuCurrentPage.children}" varStatus="status">
	<c:if test="${child.visible}"><li class="page_${child.name} depth-${menuDepth}${child.selected?' selected':''}${child.lastSelected?' last':''} item-${index}"><a href="${child.url}">${child.info.label}</a>
