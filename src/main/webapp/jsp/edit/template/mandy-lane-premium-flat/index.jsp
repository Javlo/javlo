			...
			<div class="header">
				...
				<c:if test="${currentModule.search}">
					<form id="search" action="${info.currentURL}" method="post">
						<input type="hidden" name="webaction" value="search" /> <input type="text" name="query" />
				...
				<div class="topheader">
					<ul class="notebutton">
						<c:if test="${userInterface.IM}">
							<li class="note">
								<div class="im-wizz-message" style="display: none;" title="${i18n.edit['im.title']}">${i18n.edit['im.message.wizz']}</div> <a href="${info.editTemplateURL}/im.jsp" class="messagenotify"> <span class="wrap"> <span
										class="thicon msgicon"></span> <span class="count" style="display: none;">0</span>
								</span>
							</a>
							</li>
						</c:if>
						<li class="note"><a href="${info.editTemplateURL}/notifications.jsp" class="alertnotify"> <span class="wrap"> <span class="thicon commenticon"></span> <c:if test="${not empty notificationSize}">
										<span id="notification-count" class="count">${notificationSize}</span>
									</c:if>
							</span>
						</a></li>
						<c:if test="${not empty info.privateHelpURL}">
							<li class="note"><a href="${info.privateHelpURL}" target="_blanck"> <span class="wrap"> <span class="thicon helpicon"></span> <span class="count" style="display: none;">0</span>
								</span>
							</a></li>
						</c:if>
						<c:if test="${not empty integrities}">
							<li class="note"><a href="${info.editTemplateURL}/integrity.jsp?path=${info.path}" class="alertintegrity"> <span class="wrap"> <span class="thicon infoicon"></span> <span id="integrity-count"
						...
						<c:forEach var="module" items="${modules}">
							<c:if test="${empty module.parent}">
								<li class="module ${module.name} ${module.name == currentModule.name || module.name == currentModule.parent?'current':''} ${module.name == fromModule.name?'from':''}"><c:url var="moduleURL" value="${info.currentURL}" context="/">
									...
									</c:url> <a href="${moduleURL}"><span class="title">${module.title}</span> ${module.name == currentModule.name || module.name == currentModule.parent?'<div id="ajax-loader"></div>':''} <c:if
