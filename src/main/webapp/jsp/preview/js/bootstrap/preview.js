	...
	editPreview.initPreview = function() {
		...
		if (pjq("#preview-layer").length == 0) {
			pjq("body").append('<div id="preview-layer"><div class="layer-header">'+
				'<span class=\"area-name\"><h4></h4><div class="repeat-icon"><i class="bi bi-repeat"></i></div><div class="commands btn-group btn-group-sm area-actions" role="group"><span class="mirror glyphicon glyphicon-paste" aria-hidden="true"></span><i class="bi bi-layout-sidebar"></i><span id=\"area-name\"></span></span>' +
	...
	editPreview.collapse = function() {
		$('.prvCollapse').each(function() {
			...
			colLink.click(function() {
				...
				let targetClose = pjq(pjq(this).attr('href') + ' .close-collapse');
				if (targetClose != null) {
					targetClose.data("targetCollapseId", pjq(this).attr('href'));
					targetClose.click(function() {
						let collapseWrapper = pjq(this).data("targetCollapseId");
						pjq(collapseWrapper).addClass("hiddenCollapse");
					});
				}
