<%@ taglib uri="jakarta.tags.core" prefix="c"%><%@ taglib uri="jakarta.tags.functions" prefix="fn"%><div class="${contentContext.editPreview?'preview ':'edit '}content wizard step-2">
		...
		<div class="main">
			...
			<div class="action">
				<div class="btn-group pull-right">
					...
					<button class="btn btn-primary  mb-3" type="submit" name="next">next</button>
				</div>
			</div>
			...
			<div class="form-group">
				...
				<c:if test="${empty senders}">
					<input type="text" id="mailing-sender" name="sender" class="form-control" value="<c:out value="${mailing.sender}" escapeXml="true" />" />
				</c:if>
				<c:if test="${not empty senders}">
					<select id="mailing-sender" name="sender" class="form-control">
			...
			<c:if test="${globalContext.collaborativeMode && fn:length(adminGroups) > 0}">
				<div class="form-group">
			...
			<div class="checkbox">
				<label class="suffix"> <input type="checkbox" id="mailing-test-mailing" name="test-mailing" ${mailing.testMailing?'checked="checked"':''} /> ${i18n.edit['mailing.form.test-mailing']}
				</label>
			</div>
