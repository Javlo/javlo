	...
	<form id="form-edit-template" action="${info.currentURL}" class="standard-form" method="post">
		...
		<fieldset>
			...
			<c:forEach var="area" items="${currentTemplate.areas}">
				<div class="inline">
					<h4>${area}</h4>
					<span class="delete-area"><a href="${info.currentURL}?webaction=deleteArea&area=${area}&templateid=${currentTemplate.id}" title="${i18n.edit['global.delete']}">${i18n.edit['global.delete']}</a></span>
					<ul>
			...
			<div class="inline">
				<h4>
					<input class="label-inside label" type="text" name="new-area" value="${i18n.edit['template.area.create']}..." />
				</h4>
				<ul>
					<c:set var="areaFound" value="false" scope="page" />
					<c:forEach var="htmlID" items="${currentTemplate.HTMLIDS}">
