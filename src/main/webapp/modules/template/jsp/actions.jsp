...
<c:if test="${not empty templateid}">
   <a class="action-button more" href="${fileURL}&templateid=${templateid}"><span><i class="bi bi-folder"></i> ${i18n.edit['template.action.browse']}...</span></a>
   <a class="action-button more" href="${info.currentURL}?filter=&templateid=${templateid}"><span><span><i class="bi bi-pencil"></i> ${i18n.edit['template.action.filter-image']}</span></a>
   <c:if test="${fn:length(currentTemplate.CSS)>0}">
       <a class="action-button more" href="${info.currentURL}?css=${currentTemplate.CSS[0]}&templateid=${currentTemplate.name}&webaction=template.editCSS"><span><i class="bi bi-pencil"></i> CSS</span></a>
   </c:if>
   <c:if test="${fn:length(currentTemplate.htmls)>0}">
   	<a class="action-button more" href="${info.currentURL}?html=${currentTemplate.htmls[0]}&templateid=${currentTemplate.name}&webaction=template.editHTML"><span><i class="bi bi-pencil"></i> HTML</span></a>
   </c:if>

    <c:url var="commit" value="${info.currentURL}" context="/">
        <c:param name="webaction" value="template.commit"></c:param>
        <c:param name="soft" value="true"></c:param>
    ...
    <a class="action-button ajax" href="${commit}"><span class="text"><i class="bi bi-arrow-bar-up"></i> ${i18n.edit['template.action.commit-soft']}</span></a>
   ...
   <a class="action-button ajax" href="${commit}"><span class="text"><i class="bi bi-arrow-bar-up"></i> ${i18n.edit['template.action.commit']}</span></a>
   ...
   <a class="action-button ajax" href="${commitChildren}"><span><i class="bi bi-arrow-bar-up"></i> ${i18n.edit['template.action.commit-children']}</span></a>
</c:if>
