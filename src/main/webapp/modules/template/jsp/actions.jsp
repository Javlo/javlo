...
<c:if test="${not empty param.name}"><a class="action-button" href="${info.currentURL}?webaction=commit&name=${currentTemplate.name}"><span>${i18n.edit['template.action.commit']}</span></a><a class="action-button more" href="${fileURL}&name=${currentTemplate.name}"><span>${i18n.edit['template.action.browse']}...</span></a></c:if>
<div class="clear">&nbsp;</div>
