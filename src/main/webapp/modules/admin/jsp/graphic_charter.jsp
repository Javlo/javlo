...
--%>
			...
			<div class="row">
				<div class="col-md-6">
					...
					<c:if test="${not empty logoPreview}">
						...
						<div class="delete-link">
							<a href="${delURL}" title="remove logo"><i class="fa fa-times" aria-hidden="true"></i></a>
						</div>
				...
				<div class="col-md-6">
					...
					<div class="row">
						<div class="col-md-6">
							<div class="checkbox">
								<label><input type="checkbox" name="fixMenu" ${currentContext.templateData.fixMenu?'checked="checked"':""} /> ${i18n.edit['admin.template.fix-menu']}</label>
							</div>
							<div class="checkbox">
								<label><input type="checkbox" name="largeMenu" ${currentContext.templateData.largeMenu?'checked="checked"':""} /> ${i18n.edit['admin.template.large-menu']}</label>
							</div>
							<div class="checkbox">
							...
							<div class="checkbox">
								<label><input class="charter-menu-position" type="checkbox" name="menuLeft" ${currentContext.templateData.menuLeft?'checked="checked"':""} onchange="resetMenuPosition(this)" /> ${i18n.edit['admin.template.menu-left']}</label>
							</div>

							<div class="checkbox">
								<label><input class="charter-menu-position" type="checkbox" name="menuRight" ${currentContext.templateData.menuRight?'checked="checked"':""} onchange="resetMenuPosition(this)" /> ${i18n.edit['admin.template.menu-right']}</label>
							</div>

							<div class="checkbox">
								<label><input class="charter-menu-position" type="checkbox" name="menuCenter" ${currentContext.templateData.menuCenter?'checked="checked"':""} onchange="resetMenuPosition(this)" /> ${i18n.edit['admin.template.menu-center']}</label>
							</div>

							<div class="checkbox">
								<label><input class="charter-menu-position" type="checkbox" name="menuAround" ${currentContext.templateData.menuAround?'checked="checked"':""} onchange="resetMenuPosition(this)" /> ${i18n.edit['admin.template.menu-between']}</label>
							</div>
						...
						<div class="col-md-6">
							...
							<div class="checkbox">
								<label><input type="checkbox" name="loginMenu" ${currentContext.templateData.loginMenu?'checked="checked"':""} /> ${i18n.edit['admin.template.login-menu']}</label>
							</div>

							<div class="checkbox">
								<label><input type="checkbox" name="fixSidebar" ${currentContext.templateData.fixSidebar?'checked="checked"':""} /> ${i18n.edit['admin.template.fix-sidebar']}</label>
							</div>

							<div class="checkbox">
								<label><input type="checkbox" name="searchMenu" id="searchMenu" ${currentContext.templateData.searchMenu?'checked="checked"':""} onchange="document.getElementById('jssearchMenu').checked=false;" /> ${i18n.edit['admin.template.search-menu']}</label>
							</div>
							<div class="checkbox">
								<label><input type="checkbox" name="jssearchMenu" id="jssearchMenu" ${currentContext.templateData.jssearchMenu?'checked="checked"':""} onchange="document.getElementById('searchMenu').checked=false;" /> ${i18n.edit['admin.template.jssearch-menu']}</label>
							</div>

							<hr />

							<div class="checkbox">
								<label><input type="checkbox" name="large" id="large-content" ${currentContext.templateData.large?'checked="checked"':""} onchange="document.getElementById('small-content').checked=false;" /> ${i18n.edit['admin.template.large']}</label>
							</div>
							<div class="checkbox">
								<label><input type="checkbox" name="small" id="small-content" ${currentContext.templateData.small?'checked="checked"':""} onchange="document.getElementById('large-content').checked=false;" /> ${i18n.edit['admin.template.small']}</label>
							</div>
	...
	function resetMenuPosition(e) {
		document.querySelectorAll(".charter-menu-position").forEach(i => {
			if (i!=e) {
				i.checked=false;
			}
		});
	...
	<c:forEach var="font" items="${fonts}">fontRef['${font}'] = '${fontsMap[font]}';</c:forEach>
</script>
			...
			updateTextFont('${currentContext.templateData.fontText}');
							...
							<select id="heading-font" name="fontHeading" onchange="updateHeadingFont(jQuery('#heading-font option:selected').text());">
								...
								<c:forEach var="font" items="${fonts}">
									<option ${currentContext.templateData.fontHeading == font?'selected="selected"':''}>${font}</option>
								</c:forEach>
							</select>
		...
		function updateTextFont(fontName) {
			...
			jQuery('#text-exemple').attr('style', 'font-family:'+fontName)
							...
							<select id="text-font" name="fontText" onchange="updateTextFont(jQuery('#text-font option:selected').text());">
								<option></option>
								<c:forEach var="font" items="${fonts}">
									<option ${currentContext.templateData.fontText == font?'selected="selected"':''}>${font}</option>
								</c:forEach>
							</select>
