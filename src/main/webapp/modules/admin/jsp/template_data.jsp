...
%><c:if test="${info.admin || not empty contentContext.currentTemplate.templateData[param.name]}"><div class="row data-line">	
		...
		</div><div class="col-xs-${info.admin?'7':'8'}">	
			<input placeholder="${i18n.edit['global.default']}" class="form-control ${param.style}" type="text" name="${param.name}" value="${param.value}" ${param.style == "color"?'size="7" maxlength="7" ':''}/>
		</div><c:if test="${info.admin}"><div class="col-xs-1 check">
