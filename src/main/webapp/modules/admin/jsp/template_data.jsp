...
%><c:if test="${info.admin || not empty contentContext.currentTemplate.templateData[param.name]}"><div class="row data-line">
	...
	<label for="<c:out value="${param.name}" />"><c:out value="${param.name}" /></label>
	</div><div class="col-xs-${info.admin?'3':'6'} col-${info.admin?'3':'6'}">
		<input placeholder="${i18n.edit['global.default']}" class="form-control <c:out value="${param.style}" />" type="text" name="<c:out value="${param.name}" />" value="${color}" ${param.style == "color"?'size="7" maxlength="7" ':''} onchange="this.style.backgroundColor=this.value" />
