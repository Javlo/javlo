<%@ taglib uri="jakarta.tags.core" prefix="c"%><%@ taglib prefix="fn" uri="jakarta.tags.functions"%><div class="widgetbox site-properties">
	<h3>
				...
				<fieldset>
					...
					<div class="row">
						<div class="col-sm-10">
							...
							<div class="form-group">
								<label>${i18n.edit['admin.form.size']}</label> <span>${currentContext.size}</span>
							</div>
							<div class="form-group">
									...
									<c:forEach var="area" items="${contentContext.currentTemplate.quietableAreaMap}" varStatus="status">
										<c:if test="${area != 'content'}">
											<div class="checkbox-inline">
												<label> <input name="quietAreas" type="checkbox" value="${area.value}" ${not empty currentContext.quietAreaMap[area.value]?'checked="checked"':''} /> ${area.value}
												</label>
											</div>
										</c:if>
									</c:forEach>
		...
		<c:if test="${info.admin}">
			<form id="form-page-properties" action="${info.currentURL}" method="post" enctype="multipart/form-data">
				...
				<div class="row">
					...
					<div class="col-xs-4">
						<select id="default-template" name="default-template" class="form-control">
							<option value="">&nbsp;</option>
				...
				<fieldset class="collapsable">
					...
					<div class="row">
						<div class="col-sm-10">
							...
							<div class="form-group">
								<label>${i18n.edit['admin.title.folder']}</label> <span>${currentContext.folder}</span>
							</div>
							<div class="form-group">
								<label>${i18n.edit['admin.form.size']}</label> <span>${currentContext.size}</span>
							</div>
...
<fieldset class="collapsable">
	...
	<div class="row">
		...
		<div class="col-sm-6">
			...
			<div class="checkbox">
				<label><input type="checkbox" id="only-creator-modify" name="only-creator-modify" ${currentContext.onlyCreatorModify?'checked="checked"':""} /> ${i18n.edit['admin.form.only-creator-modify']}</label>
			</div>

			<div class="checkbox">
				<label><input type="checkbox" id="collaborative-mode" name="collaborative-mode" ${currentContext.collaborativeMode?'checked="checked"':""} /> ${i18n.edit['admin.form.collaborative-mode']}</label>
			</div>

			<div class="checkbox">
				<label><input type="checkbox" id="security-forced-https" name="security-forced-https" ${currentContext.forcedHttps?'checked="checked"':""} /> ${i18n.edit['admin.form.security-forced-https']}</label>
			</div>

			<div class="checkbox">
				<label><input type="checkbox" id="security-backup-thread" name="security-backup-thread" ${currentContext.backupThread?'checked="checked"':""} /> ${i18n.edit['admin.form.security-backup-thread']}</label>
			</div>

			<div class="checkbox">
				<label><input type="checkbox" id="security-portail" name="security-portail" ${currentContext.portail?'checked="checked"':""} /> ${i18n.edit['admin.form.security-portail']}</label>
			</div>
						...
						jQuery('#qrcode')
								.click(
										function() {
											...
											item
													.wrap("<div id=\"qrcode-wrapper\" />");
											jQuery("#qrcode-wrapper")
													.html(
															'<a href="${editAutoURL}"><img src="${qrcode}" /></a>');
										});
					</script>
				</div>
			</c:if>
...
<fieldset class="macros collapsable">
	<legend>${i18n.edit['admin.title.macros']}</legend>
	<c:forEach var="macro" items="${macros}">
		<div class="col-lg-3 col-sm-6 small-col">
			<div class="checkbox">
				<label title="${macro.info}"><input type="checkbox" id="${macro}" name="${macro}" ${not empty selectedMacros[macro.name]?'checked="checked"':''} /> <c:set var="i18nKey" value="macro.${macro.label}" /> ${not empty i18n.edit[i18nKey]?i18n.edit[i18nKey]:macro.label}</label>
			</div>
		</div>
	</c:forEach>
</fieldset>
				...
				<c:forEach var="template" items="${templates}">
					<li>
						<div class="thumb">
							...
							<div class="info">
								<p>
									<label>${i18n.edit['global.name']}</label> <span>${template.name}</span>
								</p>
								<p>
									<label>${i18n.edit['global.date']}</label> <span>${template.creationDate}</span>
								</p>
								<c:if test="${!template.template.fake}">
									<p>
...
<fieldset class="templates-plugin collapsable">
