							...
							<form id="restore-${persistence.version}" method="post">
								<div class="line action">
									<input type="hidden" name="webaction" value="restore" /> <input
										type="hidden" name="version" value="${persistence.version}" />
									<input type="submit" class="warning needconfirm"
										value="${i18n.edit['persistence.title.restore']}" />
								</div>
							</form>
						</c:if>
						<c:if test="${persistence.type != 'preview'}">
							<form id="restore-${persistence.version}" method="post">
								<div class="line action">
									<input type="hidden" name="webaction" value="restore" /> <input
										type="hidden" name="file" value="${persistence.file}" />
									<input type="submit" class="warning needconfirm"
