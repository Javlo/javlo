...
<div id="item-wrapper-${node.id}" class="item-wrapper" draggable="true" data-id="${node.id}" data-aschild="false">
	...
	<a href="#" class="command delete" title="delete" onclick="jQuery('#input-delete').val('${param.id}'); submitTaxonomyForm(); return false;"><i class="bi bi-trash"></i></a>
	...
	<div class="item">
		...
		<li class="label id">
			...
			<li class="label path">
				<label class="lang" for="path-${param.id}">Path</label>
				<span class="text"><input readonly="readonly" type="text" id="path-${param.id}" class="hidden-input form-control" value="${param.path}" placeholder="ID" /></span>
			</li>
		<li class="label deco">
			<label class="lang" for="change-deco-${param.id}"><i class="bi bi-fonts" aria-hidden="true"></i></label>
