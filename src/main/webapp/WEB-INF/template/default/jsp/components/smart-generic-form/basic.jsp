...
<c:if test="${not empty param.line}"><input type="hidden" name="line" value="<c:out value="${param.line}" />" /></c:if>
<c:if test="${comp.captcha && not empty comp.recaptchaKey && not empty comp.recaptchaSecretKey}"><script src='https://www.google.com/recaptcha/api.js?hl=${info.requestContentLanguage}&render=${comp.recaptchaKey}'></script></c:if>
