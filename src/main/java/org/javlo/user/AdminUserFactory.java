...
public class AdminUserFactory extends UserFactory {
	...
	public static AdminUserFactory createAdminUserFactory(GlobalContext globalContext, HttpSession session) {
		 ...
		 * AdminUserFactory res = (AdminUserFactory) session.getServletContext().
		 * getAttribute(staticConfig.getAdminUserFactoryClassName()); if (res == null) {
		 * try { res = staticConfig.getAdminUserFactory(globalContext, session);
		 * logger.fine("create userFactory : " + res.getClass().getName()); } catch
		 * (Exception e) { logger.severe(e.getMessage()); e.printStackTrace(); }
		 * res.init(globalContext, session); session.getServletContext().setAttribute
	...
	public User login(HttpServletRequest request, String login, String password) {
		...
		// MessageRepository.getInstance(request).setGlobalMessage(new
		// GenericMessage(i18nAccess.getText("user.change-password", "Please change you
		// password."), GenericMessage.ALERT));
		// } catch (Exception e) {
		// e.printStackTrace();
		// }
		// }
		...
		try {
			globalContext.getStaticConfig().getGeneralLister().onAdminLogin(globalContext, outUser);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * logger.info("google admin login trying : "+accessToken); HttpClient
	 * httpClient = null; try { httpClient = new URLConnectionClient(); OAuthClient
	 * oAuthClient = new OAuthClient(httpClient); Google google = new Google();
	 * StaticConfig staticConfig =
	 * StaticConfig.getInstance(request.getSession().getServletContext());
	 ...
	 * SocialUser user = google.getSocialUser(accessToken, oAuthClient); if (user ==
	 * null || user.getEmail() == null || user.getEmail().isEmpty()) {
	 * logger.warning("OAuth admin login failed with google provider"); } else {
	 * System.out.println("***** AdminUserFactory.googleLogin : name = "+user.
	 * getEmail()); //TODO: remove debug trace } } catch (Exception e) {
	 * e.printStackTrace(); } finally { if (httpClient != null) {
	 * httpClient.shutdown(); } } return null; }
	 */
	...
	public User autoLogin(HttpServletRequest request, String login) {
		...
		try {
			globalContext.getStaticConfig().getGeneralLister().onAdminLogin(globalContext, user);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * password) { User outUser = super.login(globalContext, login, password);
