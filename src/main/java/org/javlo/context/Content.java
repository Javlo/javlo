...
public class Content {
	...
	/**
	 * @Deprecated use getInstance(GlobalContext)
	 * @param request
	 * @return
	 */
	public static Content createContent(HttpServletRequest request) {
		return getInstance(GlobalContext.getInstance(request));
	}

	public static Content getInstance(GlobalContext globalContext) {
		Content content = null;
		if (globalContext != null) {
			content = (Content) globalContext.getAttribute(Content.class.getName());
		}
		if (content == null) {
			content = new Content();
			if (globalContext != null) {
				globalContext.setAttribute(Content.class.getName(), content);
			}
		}
		if (globalContext != null) {
	...
	public void releaseAll(ContentContext ctx) throws Exception {
		components.clear();
		releasePreviewNav();
		releaseViewNav(ctx);
	}
	...
	public void releaseViewNav(ContentContext ctx) throws Exception {
		// setViewNav(null);
		/*
		 * if (this.viewGlobalMap != null) { this.viewGlobalMap.clear(); }
		 */
		GlobalContext globalContext = GlobalContext.getInstance(ctx.getRequest());
