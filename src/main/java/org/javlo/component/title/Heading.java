...
public class Heading extends AbstractPropertiesComponent implements ISubTitle {
	...
	protected String getEditXHTMLCode(ContentContext ctx) throws Exception {
		...
		for (int i = 1; i < 7; i++) {
			...
			depthHTML = depthHTML + "<div class=\"_jv_btn-check\"><input id=\"" + id + "\" type=\"radio\" value=\"" + i + "\" name=\"" + getInputName(DEPTH) + "\"" + (depth == i ? " checked=\"checked\"" : "") + "/>";
	...
	public String getXHTMLId(ContentContext ctx) {
		...
		String htmlID = "HEAD_"+getComponentPosition(ctx);
