...
public class EcomActions implements IAction {
	...
	protected static Logger logger = Logger.getLogger(EditActions.class.getName());
...
//					url.append("<input type=\"hidden\" name=\"charset\" value=\"" + ContentContext.CHARACTER_ENCODING + "\">");
...
//		basket.setVATNumber(vatNumber);
	...
	private static void sendMail(ContentContext ctx, EditBasketComponent comp) throws Exception {
		...
		String adminEmail = ((EditBasketComponent) comp).getEmail();
		MailingManager mailer = MailingManager.getInstance(StaticConfig.getInstance(ctx.getRequest().getSession()));
		InternetAddress from;
		InternetAddress[] bccs = null;
		if (adminEmail.trim().length() > 0) {
			bccs = InternetAddress.parse(adminEmail);
			from = bccs[0];
		} else {
...
//			out.println("<br />" + i18nAccess.getViewText("ecom.organization") + ": " + basket.getOrganization());
//			out.println("<br />" + i18nAccess.getViewText("ecom.vatnumber") + ": " + basket.getVATNumber());
			...
			((EditBasketComponent) comp).renderBasket(ctx, out, false);

			out.close();
			String mailBody = new String(outStream.toByteArray());

			mailer.sendMailing(from, tos, bccs,
					"basket validation from " + globalContext.getGlobalTitle() + " [" + basket.getProductCount() + " products]",
					mailBody, true);
		}
	}
		...
		if (basket.isValid()) {
			...
			for (Product product : basket.getProducts()) {
				if (product.getQuantity() < 1) {
					...
					mailer.sendMail(sender[0], recipient[0], (InternetAddress[]) null, "[WARNIG] Stock out of bound", basket.getId(), false);
					break;
				}
			}
